...
export const EditKidVids = ({ setEditModal, dispatch, setReload, kidVids }) => {
  const { user } = useSelector((state) => state.user);
  const { token } = useSelector((state) => state.auth);
  const { bgColor } = useSelector((state) => state.theme);
  const { t } = useTranslation();
  ...
  const handleEditKidVids = async (data, { resetForm }) => {
    setIsLoading(true);
    try {
      if (!data?.video) {
        Toast("error", "Please upload a video");
  ...
  useEffect(() => {
    const fetchSubCategories = async () => {
      if (kidVids?.category_id) {
        const { AllCategories } = await dispatch(
          getKidVidsSubCategoriesByCategory({ token, id: kidVids?.category_id })
        ).unwrap();
    ...
    <>
      <Form
        initialValues={{
          id: kidVids?.video_id || "",
          name: kidVids?.name || "",
          description: kidVids?.description || "",
          category_id: kidVids?.category_id,
          sub_category_id: kidVids?.sub_category_id || "",
          video: kidVids?.video || "",
          thumbnail: kidVids?.thumbnail || "",
          user_id: user?.id,
        }}
        validationSchema={Yup.object().shape({
          name: Yup.string().required("Name is required"),
...
export const DeleteKidVids = ({ setDeleteModal, dispatch, setReload, id }) => {
  const { token } = useSelector((state) => state.auth);
