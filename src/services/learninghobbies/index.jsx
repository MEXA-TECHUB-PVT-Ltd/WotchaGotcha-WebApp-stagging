...
import {
  addCommentOnLearningHobbies,
  addLearningHobbies,
  deleteLearningHobbies,
  getLearningHobbiesComments,
  getLearningHobbiesLikes,
  getLearningHobbiesSubCategoriesByCategory,
  likeUnlikeLearningHobbies,
  updateLearningHobbies,
...
export const AddLearningHobbies = ({
  ...
  const handleAddLearningHobbies = async (data, { resetForm }) => {
    setIsLoading(true);
    try {
      if (!data?.video) {
      ...
      const { statusCode } = await dispatch(
        addLearningHobbies({ token, payload })
      ).unwrap();
  ...
  return (
    <>
      <Form
        ...
        onSubmit={handleAddLearningHobbies}
      >
...
export const EditLearningHobbies = ({
  setEditModal,
  dispatch,
  setReload,
  learning,
  ...
  const handleEditLearningHobbies = async (data, { resetForm }) => {
    setIsLoading(true);
    try {
      if (!data?.video) {
        Toast("error", "Please upload a video");
      ...
      const payload = { ...data, video, thumbnail };
      const { statusCode } = await dispatch(
        updateLearningHobbies({ token, payload })
      ).unwrap();
      ...
      Toast("error", error?.message || "Error updated video");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <>
      <Form
        initialValues={{
          id: learning?.video_id,
          name: learning?.name || "",
          description: learning?.description || "",
          category_id: learning?.category_id,
          sub_category_id: learning?.sub_category_id || "",
        ...
        onSubmit={handleEditLearningHobbies}
      >
        {({ handleSubmit, values, handleChange, setFieldValue }) => (
          <div className="flex-col-start gap-5">
            <div className="w-full flex items-center justify-center gap-5">
...
export const DeleteLearningHobbies = ({
  setDeleteModal,
  dispatch,
  setReload,
  id,
...
export const LearningHobbiesPlayer = ({ video, isOpen, onClose, dispatch }) => {
  const { user } = useSelector((state) => state.user);
  const { token } = useSelector((state) => state.auth);
  const { isLoading } = useSelector((state) => state.learningandhobbies);
  ...
  const handleComment = async () => {
    ...
    try {
      ...
      const { statusCode } = await dispatch(
        addCommentOnLearningHobbies({ payload, token })
      ).unwrap();
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      const { statusCode } = await dispatch(
        likeUnlikeLearningHobbies({ payload, token })
      ).unwrap();
  ...
  const getAllComments = async () => {
    ...
    try {
      const data = await dispatch(
        getLearningHobbiesComments({ token, id: video.video_id })
      ).unwrap();
