...
import {
  ...
  getFanStarComments,
  getFanStarLikes,
  getFanStarSubCategoriesByCategory,
  likeUnlikeFanStar,
  updateFanStar,
...
export const AddFanStar = ({
  ...
  const handleAddFanStar = async (data, { resetForm }) => {
    setIsLoading(true);
    try {
      if (!data?.video) {
    ...
    const fetchSubCategories = async () => {
      if (categoryId) {
        const { AllCategories } = await dispatch(
          getFanStarSubCategoriesByCategory({ token, id: categoryId })
        ).unwrap();
...
export const EditFanStar = ({ setEditModal, dispatch, setReload, fanStar }) => {
  const { user } = useSelector((state) => state.user);
  const { token } = useSelector((state) => state.auth);
  const { bgColor } = useSelector((state) => state.theme);
  const { t } = useTranslation();
  ...
  const handleEditFanStar = async (data, { resetForm }) => {
    setIsLoading(true);
    try {
      if (!data?.video) {
        Toast("error", "Please select a video");
  ...
  useEffect(() => {
    const fetchSubCategories = async () => {
      if (fanStar?.category_id) {
        const { AllCategories } = await dispatch(
          getFanStarSubCategoriesByCategory({ token, id: fanStar?.category_id })
        ).unwrap();
    ...
    <>
      <Form
        initialValues={{
          id: fanStar?.video_id || "",
          name: fanStar?.name || "",
          description: fanStar?.description || "",
          category_id: fanStar?.category_id || "",
          sub_category_id: fanStar?.sub_category_id || "",
          video: fanStar?.video || "",
          thumbnail: fanStar?.thumbnail || "",
          user_id: user?.id,
        }}
        validationSchema={Yup.object().shape({
          name: Yup.string().required("Name is required"),
...
export const DeleteFanStar = ({ setDeleteModal, dispatch, setReload, id }) => {
  const { token } = useSelector((state) => state.auth);
...
export const FanStarPlayer = ({ video, isOpen, onClose, dispatch }) => {
  ...
  const handleComment = async () => {
    ...
    try {
      ...
      const { statusCode } = await dispatch(
        addCommentOnFanStar({ payload, token })
      ).unwrap();
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      const { statusCode } = await dispatch(
        likeUnlikeFanStar({ payload, token })
      ).unwrap();
  ...
  const getAllLikes = async () => {
    ...
    try {
      const data = await dispatch(
        getFanStarLikes({ token, id: video.video_id })
      ).unwrap();
  ...
  const getAllComments = async () => {
    ...
    try {
      const data = await dispatch(
        getFanStarComments({ token, id: video.video_id })
      ).unwrap();
